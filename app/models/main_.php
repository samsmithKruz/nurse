...
class main_ extends Database
{
    ...
    public function sendContactForm($post){
        $name = Auth::safe_data($post['name']);
        $email = Auth::safe_data($post['email']);
        $phone = Auth::safe_data($post['phone']);
        $message = Auth::safe_data($post['message']);
        ...
        return Helpers::send_contact_email($name,$email,$phone,$message);
    }
}
